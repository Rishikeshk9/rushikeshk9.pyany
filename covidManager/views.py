...
def friendloc(request):
    ...
    if request.method == 'POST':
        ...
        if(userid_frnd!=None and userid_frnd!=""):
...
def sdrz(pstr):
    ...
    pstr_list = pstr.split(",")
    for i in range(len(pstr_list)):
        if(pstr_list[i]!="" and i!=len(pstr_list)-1):
            res=res+pstr_list[i]+","
...
def redzone(request):
    ...
    count1 = 0
    if request.method == "POST":
        covidred = Covidiot.objects.filter(iv__gt=iv1)
        print("Covid red obj filtered first obj id: ",covidred[0].id)

        for i in covidred:
            count1 = count1 + 1

        print("Counted items: ",count1)


        for i in range(count1):
            userid = covidred[i].id
            covidobj = Covidiot.objects.get(id=userid)
            latt = covidobj.lat
            lonn = covidobj.lon
            izv = covidobj.iv
            if(i == (count1-1) and i!=0):
                str1 = str1+str(latt)+","+str(lonn)+","+str(izv)
            else:
                str1 = str1+str(latt)+","+str(lonn)+","+str(izv)+","

            result = str1

    print("redzone str res: ", str1)
    return HttpResponse(result)


def redzonecount(request):
    str1="NULL"
    iv1 = 0.8
    count1 = 0
    value = ""
...
def fetch(request):
    ...
    if request.method == 'POST':
        userid = request.POST.get("userID")
        result_user = Covidiot.objects.get(id=userid)
        res_frnd = result_user.friends

        res_frnd_list = res_frnd.split(",")

        for i in res_frnd_list:
            if(i!=None and i!=""):
                frnd_temp = Covidiot.objects.get(id=i)
                username = frnd_temp.mob
                str1 = str1+str(i)+","+str(username)+","
    ...
    if request.method == 'POST':
        ...
        if(result_frnd!=None):
            ...
            for i in res_frd_list:
                if(i==userid):
                    main_flag=False
                    #this will check whether the sender is already your friend
        ...
        if(main_flag==True):
            ...
            print("---ADDEDD FRIENDD:")
            print("p_in: ",result_in.pending_in)
            print("p_out: ",result_out.pending_out)
            result_in.pending_in = sdrz(result_in.pending_in)
            result_in.save()

            result_out.pending_out=sdrz(result_out.pending_out)
            result_out.save()
            print("---ADDEDD FRIENDD:")
            print("p_in: ",result_in.pending_in)
            print("p_out: ",result_out.pending_out)
